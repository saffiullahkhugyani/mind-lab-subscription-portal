import { PlanCancelDataModel, PlanUpgradeDataModel } from "@/types/custom-types";
...
export async function GET() {
    ...
    try {
        const {data, error} = await supabase.from("plan-cancel-request").select();
        ...
        return NextResponse.json({error: `Failed to get list of cancel requests: ${error}`}, {status: 500});
    }
}