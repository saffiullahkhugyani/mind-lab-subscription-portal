...
export default function SendSpecialDeal({
  setIsOpen,
  users,
  programs,
  programSubscription,
  ...
  const handleSubmit = async () => {
    if (!selectedUser || !selectedProgram || !normal || !special) {
      toast({
        title: "Error",
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({
          title: "Success",
          description: `Special Deal email sent to ${selectedUser.email} successfully!`,
          variant: "success",
        });
      }
    } catch (error) {
    ...
    <div className="flex flex-row gap-6">
      ...
      <div className="flex-1 flex flex-col gap-4 mt-8 p-5">
        ...
        <div>
          ...
          <SearchableDropdown<Users>
            items={users!}
            onSelect={(user) => setSelectedUser(user)}
            getLabel={(item) => item.name!.trim()}
            getValue={(item) => item.email!.trim()}
